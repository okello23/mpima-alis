...
class HmisReportController extends Controller {
	...
	public function hmis108($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		$bloodbank_count = $this->getHMIS108serviceCounts($month);
		$bloodbank_reasons = $this->getHMIS108transfusionreasonCounts($month);
		return view('reports.hmis.hmis108')
							->with('month', $month)
							->with('bloodbank_count', $bloodbank_count)
							->with('bloodbank_reasons', $bloodbank_reasons);
	}
	public function hmis33bRegister($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.hmis33b')
							->with('month', $month);
	}
	public function biosafetyRegister($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.biosafety')
							->with('month', $month);
	}
	public function chemistryRegister($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.chemistry_register')
							->with('month', $month);
	}
	public function dailyHIVregister($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.daily_hiv_register')
							->with('month', $month);
	}
	public function EQARegister($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.eqa_register')
							->with('month', $month);
	}
	public function equipmentBreakdownRegister($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.eqt_brkdwn_register')
							->with('month', $month);
	}
	public function equipmentMaintenanceRegister($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.eqt_maintenance_register')
							->with('month', $month);
	}
	public function equipmentRegister($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.equipment_register')
							->with('month', $month);
	}
	public function haematologyRegister($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.haematology_register')
							->with('month', $month);
	}
	public function labQuaterlyReport($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.quaterly_report')
							->with('month', $month);
	}
	public function microbiologyRegister($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.microb_register')
							->with('month', $month);
	}
	public function referralRegister($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.referral_register')
							->with('month', $month);
	}
	public function VLTBregister($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.tb_register')
							->with('month', $month);
	}
	public function sampleReceptionRegister($month = '')
	{
		$month = ($month == '') ? date('Y-m') : $month ;
		return view('reports.hmis.sample_register')
							->with('month', $month);
	}
	/*
...
// GROUP BY t.specimen_id";
	...
	private function getHMIS108service($month){
		$query = "select blood_component, COUNT(bb.id) as recieved, SUM(CASE WHEN bb.status = 1 THEN 1 ELSE 0 END) as transfused FROM blood_bank bb INNER JOIN patient_transfused pt ON(pt.blood_bank_id = bb.id)
			WHERE `bb`.`created_at` LIKE '%".$month."%'
			GROUP BY blood_component";
			$rows = DB::select($query);
			return $rows;
	}

	private function getHMIS108transfusionReasons($month){
		$query = "select reason, type, SUM(CASE WHEN up.gender = 1 THEN 1 ELSE 0 END) as female, SUM(CASE WHEN up.gender = 0 THEN 1 ELSE 0 END) as male, TIMESTAMPDIFF(year, dob, CURRENT_TIMESTAMP()) AS Age, SUM(CASE WHEN up.gender = 1 AND TIMESTAMPDIFF(year, dob, CURRENT_TIMESTAMP()) < 5 THEN 1 ELSE 0 END) AS Femalebelow5, SUM(CASE WHEN up.gender = 0 AND TIMESTAMPDIFF(year, dob, CURRENT_TIMESTAMP()) < 5 THEN 1 ELSE 0 END) AS malebelow5, SUM(CASE WHEN up.gender = 1 AND TIMESTAMPDIFF(year, dob, CURRENT_TIMESTAMP()) > 4 THEN 1 ELSE 0 END) AS Femaleabove5, SUM(CASE WHEN up.gender = 0 AND TIMESTAMPDIFF(year, dob, CURRENT_TIMESTAMP()) > 4 THEN 1 ELSE 0 END) AS maleabove5 FROM patient_transfused pt INNER JOIN blood_bank bb ON(pt.blood_bank_id = bb.id) INNER JOIN unhls_patients up ON (up.id = pt.patient_id)
			WHERE `bb`.`created_at` LIKE '%".$month."%'
			GROUP BY reason,type, up.dob";
			$rows = DB::select($query);
			return $rows;
	}

	private function getHMIS108serviceCounts($month){
		$rows = $this->getHMIS108service($month);
		//$test_type_counts = $this->getTestTypeInitialCounts('hematology');
		$bloodbank_counts['wholeblood_request'] = 0;
		$bloodbank_counts['wholeblood_received'] = 0;
		$bloodbank_counts['wholeblood_transfused'] = 0;
		$bloodbank_counts['wholeblood_reactions'] = 0;
		$bloodbank_counts['packedcells_request'] = 0;
		$bloodbank_counts['packedcells_received'] = 0;
		$bloodbank_counts['packedcells_transfused'] = 0;
		$bloodbank_counts['packedcells_reactions'] = 0;
		$bloodbank_counts['platelets_request'] = 0;
		$bloodbank_counts['platelets_received'] = 0;
		$bloodbank_counts['platelets_transfused'] = 0;
		$bloodbank_counts['platelets_reactions'] = 0;
		$bloodbank_counts['ffp_request'] = 0;
		$bloodbank_counts['ffp_received'] = 0;
		$bloodbank_counts['ffp_transfused'] = 0;
		$bloodbank_counts['ffp_reactions'] = 0;
		$bloodbank_counts['cryo_request'] = 0;
		$bloodbank_counts['cryo_received'] = 0;
		$bloodbank_counts['cryo_transfused'] = 0;
		$bloodbank_counts['cryo_reactions'] = 0;
		
	 	foreach($rows as $row){
	 		//dd($row);
	 		if($row->blood_component == 'Whole blood'){
	 			$bloodbank_counts['wholeblood_request'] = 0;
	 			$bloodbank_counts['wholeblood_received'] = $row->recieved;
	 			$bloodbank_counts['wholeblood_transfused'] = $row->transfused;
	 			$bloodbank_counts['wholeblood_reactions'] = '';
	 		}else if($row->blood_component == 'Packed cells'){
	 			$bloodbank_counts['packedcells_request'] = 0;
	 			$bloodbank_counts['packedcells_received'] = $row->recieved;
	 			$bloodbank_counts['packedcells_transfused'] = $row->transfused;
	 			$bloodbank_counts['packedcells_reactions'] = '';
	 		}else if($row->blood_component == 'FFP'){
	 			$bloodbank_counts['ffp_request'] = 0;
	 			$bloodbank_counts['ffp_received'] = $row->recieved;
	 			$bloodbank_counts['ffp_transfused'] = $row->transfused;
	 			$bloodbank_counts['ffp_reactions'] = '';
	 		}else if($row->blood_component == 'CRYO'){
	 			$bloodbank_counts['cryo_request'] = 0;
	 			$bloodbank_counts['cryo_received'] = $row->recieved;
	 			$bloodbank_counts['cryo_transfused'] = $row->transfused;
	 			$bloodbank_counts['cryo_reactions'] = '';
	 		}else{
	 			$bloodbank_counts['platelets_request'] = 0;
	 			$bloodbank_counts['platelets_received'] = $row->recieved;
	 			$bloodbank_counts['platelets_transfused'] = $row->transfused;
	 			$bloodbank_counts['platelets_reactions'] = '';
	 		}

	 	}
	 	
	return $bloodbank_counts++;
	}

	private function getHMIS108transfusionreasonCounts($month){
		$rows = $this->getHMIS108transfusionReasons($month);
		//$test_type_counts = $this->getTestTypeInitialCounts('hematology');
		$bloodbank_counts['malaria_f_belowfour'] = 0;
		$bloodbank_counts['malaria_f_belowfour2'] = 0;
		$bloodbank_counts['malaria_f_belowfour3'] = 0;
		$bloodbank_counts['malaria_f_belowfour4'] = 0;
		$bloodbank_counts['malaria_f_belowfour5'] = 0;
		$bloodbank_counts['malaria_m_belowfour'] = 0;
		$bloodbank_counts['malaria_m_belowfour2'] = 0;
		$bloodbank_counts['malaria_m_belowfour3'] = 0;
		$bloodbank_counts['malaria_m_belowfour4'] = 0;
		$bloodbank_counts['malaria_m_belowfour5'] = 0;
		$bloodbank_counts['malaria_f_abovefour'] = 0;
		$bloodbank_counts['malaria_f_abovefour2'] = 0;
		$bloodbank_counts['malaria_f_abovefour3'] = 0;
		$bloodbank_counts['malaria_f_abovefour4'] = 0;
		$bloodbank_counts['malaria_f_abovefour5'] = 0;
		$bloodbank_counts['malaria_m_abovefour'] = 0;
		$bloodbank_counts['malaria_m_abovefour2'] = 0;
		$bloodbank_counts['malaria_m_abovefour3'] = 0;
		$bloodbank_counts['malaria_m_abovefour4'] = 0;
		$bloodbank_counts['malaria_m_abovefour5'] = 0;
		$bloodbank_counts['sicklecells_f_belowfour'] = 0;
		$bloodbank_counts['sicklecells_m_belowfour'] = 0;
		$bloodbank_counts['sicklecells_f_abovefour'] = 0;
		$bloodbank_counts['sicklecells_m_abovefour'] = 0;
		$bloodbank_counts['sicklecells_f_belowfour2'] = 0;
		$bloodbank_counts['sicklecells_m_belowfour2'] = 0;
		$bloodbank_counts['sicklecells_f_abovefour2'] = 0;
		$bloodbank_counts['sicklecells_m_abovefour2'] = 0;
		$bloodbank_counts['sicklecells_f_belowfour3'] = 0;
		$bloodbank_counts['sicklecells_m_belowfour3'] = 0;
		$bloodbank_counts['sicklecells_f_abovefour3'] = 0;
		$bloodbank_counts['sicklecells_m_abovefour3'] = 0;
		$bloodbank_counts['sicklecells_f_belowfour4'] = 0;
		$bloodbank_counts['sicklecells_m_belowfour4'] = 0;
		$bloodbank_counts['sicklecells_f_abovefour4'] = 0;
		$bloodbank_counts['sicklecells_m_abovefour4'] = 0;
		$bloodbank_counts['sicklecells_f_belowfour5'] = 0;
		$bloodbank_counts['sicklecells_m_belowfour5'] = 0;
		$bloodbank_counts['sicklecells_f_abovefour5'] = 0;
		$bloodbank_counts['sicklecells_m_abovefour5'] = 0;
		$bloodbank_counts['pregnancy_f_belowfour'] = 0;
		$bloodbank_counts['pregnancy_m_belowfour'] = 0;
		$bloodbank_counts['pregnancy_f_abovefour'] = 0;
		$bloodbank_counts['pregnancy_m_abovefour'] = 0;
		$bloodbank_counts['pregnancy_f_belowfour2'] = 0;
		$bloodbank_counts['pregnancy_m_belowfour2'] = 0;
		$bloodbank_counts['pregnancy_f_abovefour2'] = 0;
		$bloodbank_counts['pregnancy_m_abovefour2'] = 0;
		$bloodbank_counts['pregnancy_f_belowfour3'] = 0;
		$bloodbank_counts['pregnancy_m_belowfour3'] = 0;
		$bloodbank_counts['pregnancy_f_abovefour3'] = 0;
		$bloodbank_counts['pregnancy_m_abovefour3'] = 0;
		$bloodbank_counts['pregnancy_f_belowfour4'] = 0;
		$bloodbank_counts['pregnancy_m_belowfour4'] = 0;
		$bloodbank_counts['pregnancy_f_abovefour4'] = 0;
		$bloodbank_counts['pregnancy_m_abovefour4'] = 0;
		$bloodbank_counts['pregnancy_f_belowfour5'] = 0;
		$bloodbank_counts['pregnancy_m_belowfour5'] = 0;
		$bloodbank_counts['pregnancy_f_abovefour5'] = 0;
		$bloodbank_counts['pregnancy_m_abovefour5'] = 0;
		$bloodbank_counts['trauma_f_belowfour'] = 0;
		$bloodbank_counts['trauma_m_belowfour'] = 0;
		$bloodbank_counts['trauma_f_abovefour'] = 0;
		$bloodbank_counts['trauma_m_abovefour'] = 0;
		$bloodbank_counts['trauma_f_belowfour2'] = 0;
		$bloodbank_counts['trauma_m_belowfour2'] = 0;
		$bloodbank_counts['trauma_f_abovefour2'] = 0;
		$bloodbank_counts['trauma_m_abovefour2'] = 0;
		$bloodbank_counts['trauma_f_belowfour3'] = 0;
		$bloodbank_counts['trauma_m_belowfour3'] = 0;
		$bloodbank_counts['trauma_f_abovefour3'] = 0;
		$bloodbank_counts['trauma_m_abovefour3'] = 0;
		$bloodbank_counts['trauma_f_belowfour4'] = 0;
		$bloodbank_counts['trauma_m_belowfour4'] = 0;
		$bloodbank_counts['trauma_f_abovefour4'] = 0;
		$bloodbank_counts['trauma_m_abovefour4'] = 0;
		$bloodbank_counts['trauma_f_belowfour5'] = 0;
		$bloodbank_counts['trauma_m_belowfour5'] = 0;
		$bloodbank_counts['trauma_f_abovefour5'] = 0;
		$bloodbank_counts['trauma_m_abovefour5'] = 0;
		$bloodbank_counts['accident_f_belowfour'] = 0;
		$bloodbank_counts['accident_m_belowfour'] = 0;
		$bloodbank_counts['accident_f_abovefour'] = 0;
		$bloodbank_counts['accident_m_abovefour'] = 0;
		$bloodbank_counts['accident_f_belowfour2'] = 0;
		$bloodbank_counts['accident_m_belowfour2'] = 0;
		$bloodbank_counts['accident_f_abovefour2'] = 0;
		$bloodbank_counts['accident_m_abovefour2'] = 0;
		$bloodbank_counts['accident_f_belowfour3'] = 0;
		$bloodbank_counts['accident_m_belowfour3'] = 0;
		$bloodbank_counts['accident_f_abovefour3'] = 0;
		$bloodbank_counts['accident_m_abovefour3'] = 0;
		$bloodbank_counts['accident_f_belowfour4'] = 0;
		$bloodbank_counts['accident_m_belowfour4'] = 0;
		$bloodbank_counts['accident_f_abovefour4'] = 0;
		$bloodbank_counts['accident_m_abovefour4'] = 0;
		$bloodbank_counts['accident_f_belowfour5'] = 0;
		$bloodbank_counts['accident_m_belowfour5'] = 0;
		$bloodbank_counts['accident_f_abovefour5'] = 0;
		$bloodbank_counts['accident_m_abovefour5'] = 0;
		$bloodbank_counts['IDA_f_belowfour'] = 0;
		$bloodbank_counts['IDA_m_belowfour'] = 0;
		$bloodbank_counts['IDA_f_abovefour'] = 0;
		$bloodbank_counts['IDA_m_abovefour'] = 0;
		$bloodbank_counts['IDA_f_belowfour2'] = 0;
		$bloodbank_counts['IDA_m_belowfour2'] = 0;
		$bloodbank_counts['IDA_f_abovefour2'] = 0;
		$bloodbank_counts['IDA_m_abovefour2'] = 0;
		$bloodbank_counts['IDA_f_belowfour3'] = 0;
		$bloodbank_counts['IDA_m_belowfour3'] = 0;
		$bloodbank_counts['IDA_f_abovefour3'] = 0;
		$bloodbank_counts['IDA_m_abovefour3'] = 0;
		$bloodbank_counts['IDA_f_belowfour4'] = 0;
		$bloodbank_counts['IDA_m_belowfour4'] = 0;
		$bloodbank_counts['IDA_f_abovefour4'] = 0;
		$bloodbank_counts['IDA_m_abovefour4'] = 0;
		$bloodbank_counts['IDA_f_belowfour5'] = 0;
		$bloodbank_counts['IDA_m_belowfour5'] = 0;
		$bloodbank_counts['IDA_f_abovefour5'] = 0;
		$bloodbank_counts['IDA_m_abovefour5'] = 0;
		$bloodbank_counts['cancer_f_belowfour'] = 0;
		$bloodbank_counts['cancer_m_belowfour'] = 0;
		$bloodbank_counts['cancer_f_abovefour'] = 0;
		$bloodbank_counts['cancer_m_abovefour'] = 0;
		$bloodbank_counts['cancer_f_belowfour2'] = 0;
		$bloodbank_counts['cancer_m_belowfour2'] = 0;
		$bloodbank_counts['cancer_f_abovefour2'] = 0;
		$bloodbank_counts['cancer_m_abovefour2'] = 0;
		$bloodbank_counts['cancer_f_belowfour3'] = 0;
		$bloodbank_counts['cancer_m_belowfour3'] = 0;
		$bloodbank_counts['cancer_f_abovefour3'] = 0;
		$bloodbank_counts['cancer_m_abovefour3'] = 0;
		$bloodbank_counts['cancer_f_belowfour4'] = 0;
		$bloodbank_counts['cancer_m_belowfour4'] = 0;
		$bloodbank_counts['cancer_f_abovefour4'] = 0;
		$bloodbank_counts['cancer_m_abovefour4'] = 0;
		$bloodbank_counts['cancer_f_belowfour5'] = 0;
		$bloodbank_counts['cancer_m_belowfour5'] = 0;
		$bloodbank_counts['cancer_f_abovefour5'] = 0;
		$bloodbank_counts['cancer_m_abovefour5'] = 0;

	 	foreach($rows as $row){
		$blood_type = $row->type;
	 		//dd($row);
	 		if($row->reason == 'Malaria' && $blood_type == 'Whole blood'){
	 			$bloodbank_counts['malaria_f_belowfour'] = $row->Femalebelow5;
	 			$bloodbank_counts['malaria_m_belowfour'] = $row->malebelow5;
	 			$bloodbank_counts['malaria_f_abovefour'] = $row->Femaleabove5;
	 			$bloodbank_counts['malaria_m_abovefour'] = $row->maleabove5;
	 		}else if($row->reason == 'sickle-cell-anaemia' && $blood_type == 'Whole blood'){
	 			$bloodbank_counts['sicklecells_f_belowfour'] = $row->Femalebelow5;
	 			$bloodbank_counts['sicklecells_m_belowfour'] = $row->malebelow5;
	 			$bloodbank_counts['sicklecells_f_abovefour'] = $row->Femaleabove5;
	 			$bloodbank_counts['sicklecells_m_abovefour'] = $row->maleabove5;
	 		}else if($row->reason == 'Pregnancy-related' && $blood_type == 'Whole blood'){
	 			$bloodbank_counts['pregnancy_f_belowfour'] = $row->Femalebelow5;
	 			$bloodbank_counts['pregnancy_m_belowfour'] = $row->malebelow5;
	 			$bloodbank_counts['pregnancy_f_abovefour'] = $row->Femaleabove5;
	 			$bloodbank_counts['pregnancy_m_abovefour'] = $row->maleabove5;
	 		}else if($row->reason == 'Trauma' && $blood_type == 'Whole blood'){
	 			$bloodbank_counts['trauma_f_belowfour'] = $row->Femalebelow5;
	 			$bloodbank_counts['trauma_m_belowfour'] = $row->malebelow5;
	 			$bloodbank_counts['trauma_f_abovefour'] = $row->Femaleabove5;
	 			$bloodbank_counts['trauma_m_abovefour'] = $row->maleabove5;
	 		}else if($row->reason == 'Accident' && $blood_type == 'Whole blood'){
	 			$bloodbank_counts['accident_f_belowfour'] = $row->Femalebelow5;
	 			$bloodbank_counts['accident_m_belowfour'] = $row->malebelow5;
	 			$bloodbank_counts['accident_f_abovefour'] = $row->Femaleabove5;
	 			$bloodbank_counts['accident_m_abovefour'] = $row->maleabove5;
	 		}else if($row->reason == 'IDA' && $blood_type == 'Whole blood'){
	 			$bloodbank_counts['IDA_f_belowfour'] = $row->Femalebelow5;
	 			$bloodbank_counts['IDA_m_belowfour'] = $row->malebelow5;
	 			$bloodbank_counts['IDA_f_abovefour'] = $row->Femaleabove5;
	 			$bloodbank_counts['IDA_m_abovefour'] = $row->maleabove5;
	 		}else if($row->reason == 'cancer' && $blood_type == 'Whole blood'){
	 			$bloodbank_counts['cancer_f_belowfour'] = $row->Femalebelow5;
	 			$bloodbank_counts['cancer_m_belowfour'] = $row->malebelow5;
	 			$bloodbank_counts['cancer_f_abovefour'] = $row->Femaleabove5;
	 			$bloodbank_counts['cancer_m_abovefour'] = $row->maleabove5;
	 		}if($row->reason == 'Malaria' && $blood_type == 'packed cells'){
	 			$bloodbank_counts['malaria_f_belowfour2'] = $row->Femalebelow5;
	 			$bloodbank_counts['malaria_m_belowfour2'] = $row->malebelow5;
	 			$bloodbank_counts['malaria_f_abovefour2'] = $row->Femaleabove5;
	 			$bloodbank_counts['malaria_m_abovefour2'] = $row->maleabove5;
	 		}else if($row->reason == 'sickle-cell-anaemia' && $blood_type == 'packed cells'){
	 			$bloodbank_counts['sicklecells_f_belowfour2'] = $row->Femalebelow5;
	 			$bloodbank_counts['sicklecells_m_belowfour2'] = $row->malebelow5;
	 			$bloodbank_counts['sicklecells_f_abovefour2'] = $row->Femaleabove5;
	 			$bloodbank_counts['sicklecells_m_abovefour2'] = $row->maleabove5;
	 		}else if($row->reason == 'Pregnancy-related' && $blood_type == 'packed cells'){
	 			$bloodbank_counts['pregnancy_f_belowfour2'] = $row->Femalebelow5;
	 			$bloodbank_counts['pregnancy_m_belowfour2'] = $row->malebelow5;
	 			$bloodbank_counts['pregnancy_f_abovefour2'] = $row->Femaleabove5;
	 			$bloodbank_counts['pregnancy_m_abovefour2'] = $row->maleabove5;
	 		}else if($row->reason == 'Trauma' && $blood_type == 'packed cells'){
	 			$bloodbank_counts['trauma_f_belowfour2'] = $row->Femalebelow5;
	 			$bloodbank_counts['trauma_m_belowfour2'] = $row->malebelow5;
	 			$bloodbank_counts['trauma_f_abovefour2'] = $row->Femaleabove5;
	 			$bloodbank_counts['trauma_m_abovefour2'] = $row->maleabove5;
	 		}else if($row->reason == 'Accident' && $blood_type == 'packed cells'){
	 			$bloodbank_counts['accident_f_belowfour2'] = $row->Femalebelow5;
	 			$bloodbank_counts['accident_m_belowfour2'] = $row->malebelow5;
	 			$bloodbank_counts['accident_f_abovefour2'] = $row->Femaleabove5;
	 			$bloodbank_counts['accident_m_abovefour2'] = $row->maleabove5;
	 		}else if($row->reason == 'IDA' && $blood_type == 'packed cells'){
	 			$bloodbank_counts['IDA_f_belowfour2'] = $row->Femalebelow5;
	 			$bloodbank_counts['IDA_m_belowfour2'] = $row->malebelow5;
	 			$bloodbank_counts['IDA_f_abovefour2'] = $row->Femaleabove5;
	 			$bloodbank_counts['IDA_m_abovefour2'] = $row->maleabove5;
	 		}else if($row->reason == 'cancer' && $blood_type == 'packed cells'){
	 			$bloodbank_counts['cancer_f_belowfour2'] = $row->Femalebelow5;
	 			$bloodbank_counts['cancer_m_belowfour2'] = $row->malebelow5;
	 			$bloodbank_counts['cancer_f_abovefour2'] = $row->Femaleabove5;
	 			$bloodbank_counts['cancer_m_abovefour2'] = $row->maleabove5;
	 		}if($row->reason == 'Malaria' && $blood_type == 'Platelets'){
	 			$bloodbank_counts['malaria_f_belowfour'] = $row->Femalebelow5;
	 			$bloodbank_counts['malaria_m_belowfour'] = $row->malebelow5;
	 			$bloodbank_counts['malaria_f_abovefour'] = $row->Femaleabove5;
	 			$bloodbank_counts['malaria_m_abovefour'] = $row->maleabove5;
	 		}else if($row->reason == 'sickle-cell-anaemia' && $blood_type == 'Platelets'){
	 			$bloodbank_counts['sicklecells_f_belowfour3'] = $row->Femalebelow5;
	 			$bloodbank_counts['sicklecells_m_belowfour3'] = $row->malebelow5;
	 			$bloodbank_counts['sicklecells_f_abovefour3'] = $row->Femaleabove5;
	 			$bloodbank_counts['sicklecells_m_abovefour3'] = $row->maleabove5;
	 		}else if($row->reason == 'Pregnancy-related' && $blood_type == 'Platelets'){
	 			$bloodbank_counts['pregnancy_f_belowfour3'] = $row->Femalebelow5;
	 			$bloodbank_counts['pregnancy_m_belowfour3'] = $row->malebelow5;
	 			$bloodbank_counts['pregnancy_f_abovefour3'] = $row->Femaleabove5;
	 			$bloodbank_counts['pregnancy_m_abovefour3'] = $row->maleabove5;
	 		}else if($row->reason == 'Trauma' && $blood_type == 'Platelets'){
	 			$bloodbank_counts['trauma_f_belowfour3'] = $row->Femalebelow5;
	 			$bloodbank_counts['trauma_m_belowfour3'] = $row->malebelow5;
	 			$bloodbank_counts['trauma_f_abovefour3'] = $row->Femaleabove5;
	 			$bloodbank_counts['trauma_m_abovefour3'] = $row->maleabove5;
	 		}else if($row->reason == 'Accident' && $blood_type == 'Platelets'){
	 			$bloodbank_counts['accident_f_belowfour3'] = $row->Femalebelow5;
	 			$bloodbank_counts['accident_m_belowfour3'] = $row->malebelow5;
	 			$bloodbank_counts['accident_f_abovefour3'] = $row->Femaleabove5;
	 			$bloodbank_counts['accident_m_abovefour3'] = $row->maleabove5;
	 		}else if($row->reason == 'IDA' && $blood_type == 'Platelets'){
	 			$bloodbank_counts['IDA_f_belowfour3'] = $row->Femalebelow5;
	 			$bloodbank_counts['IDA_m_belowfour3'] = $row->malebelow5;
	 			$bloodbank_counts['IDA_f_abovefour3'] = $row->Femaleabove5;
	 			$bloodbank_counts['IDA_m_abovefour3'] = $row->maleabove5;
	 		}else if($row->reason == 'cancer' && $blood_type == 'Platelets'){
	 			$bloodbank_counts['cancer_f_belowfour3'] = $row->Femalebelow5;
	 			$bloodbank_counts['cancer_m_belowfour3'] = $row->malebelow5;
	 			$bloodbank_counts['cancer_f_abovefour3'] = $row->Femaleabove5;
	 			$bloodbank_counts['cancer_m_abovefour3'] = $row->maleabove5;
	 		}if($row->reason == 'Malaria' && $blood_type == 'FFP'){
	 			$bloodbank_counts['malaria_f_belowfour4'] = $row->Femalebelow5;
	 			$bloodbank_counts['malaria_m_belowfour4'] = $row->malebelow5;
	 			$bloodbank_counts['malaria_f_abovefour4'] = $row->Femaleabove5;
	 			$bloodbank_counts['malaria_m_abovefour4'] = $row->maleabove5;
	 		}else if($row->reason == 'sickle-cell-anaemia' && $blood_type == 'FFP'){
	 			$bloodbank_counts['sicklecells_f_belowfour4'] = $row->Femalebelow5;
	 			$bloodbank_counts['sicklecells_m_belowfour4'] = $row->malebelow5;
	 			$bloodbank_counts['sicklecells_f_abovefour4'] = $row->Femaleabove5;
	 			$bloodbank_counts['sicklecells_m_abovefour4'] = $row->maleabove5;
	 		}else if($row->reason == 'Pregnancy-related' && $blood_type == 'FFP'){
	 			$bloodbank_counts['pregnancy_f_belowfour4'] = $row->Femalebelow5;
	 			$bloodbank_counts['pregnancy_m_belowfour4'] = $row->malebelow5;
	 			$bloodbank_counts['pregnancy_f_abovefour4'] = $row->Femaleabove5;
	 			$bloodbank_counts['pregnancy_m_abovefour4'] = $row->maleabove5;
	 		}else if($row->reason == 'Trauma' && $blood_type == 'FFP'){
	 			$bloodbank_counts['trauma_f_belowfour4'] = $row->Femalebelow5;
	 			$bloodbank_counts['trauma_m_belowfour4'] = $row->malebelow5;
	 			$bloodbank_counts['trauma_f_abovefour4'] = $row->Femaleabove5;
	 			$bloodbank_counts['trauma_m_abovefour4'] = $row->maleabove5;
	 		}else if($row->reason == 'Accident' && $blood_type == 'FFP'){
	 			$bloodbank_counts['accident_f_belowfour4'] = $row->Femalebelow5;
	 			$bloodbank_counts['accident_m_belowfour4'] = $row->malebelow5;
	 			$bloodbank_counts['accident_f_abovefour4'] = $row->Femaleabove5;
	 			$bloodbank_counts['accident_m_abovefour4'] = $row->maleabove5;
	 		}else if($row->reason == 'IDA' && $blood_type == 'FFP'){
	 			$bloodbank_counts['IDA_f_belowfour4'] = $row->Femalebelow5;
	 			$bloodbank_counts['IDA_m_belowfour4'] = $row->malebelow5;
	 			$bloodbank_counts['IDA_f_abovefour4'] = $row->Femaleabove5;
	 			$bloodbank_counts['IDA_m_abovefour4'] = $row->maleabove5;
	 		}else if($row->reason == 'cancer' && $blood_type == 'FFP'){
	 			$bloodbank_counts['cancer_f_belowfour4'] = $row->Femalebelow5;
	 			$bloodbank_counts['cancer_m_belowfour4'] = $row->malebelow5;
	 			$bloodbank_counts['cancer_f_abovefour4'] = $row->Femaleabove5;
	 			$bloodbank_counts['cancer_m_abovefour4'] = $row->maleabove5;
	 		}if($row->reason == 'Malaria' && $blood_type == 'CRYO'){
	 			$bloodbank_counts['malaria_f_belowfour5'] = $row->Femalebelow5;
	 			$bloodbank_counts['malaria_m_belowfour5'] = $row->malebelow5;
	 			$bloodbank_counts['malaria_f_abovefour5'] = $row->Femaleabove5;
	 			$bloodbank_counts['malaria_m_abovefour5'] = $row->maleabove5;
	 		}else if($row->reason == 'sickle-cell-anaemia' && $blood_type == 'CRYO'){
	 			$bloodbank_counts['sicklecells_f_belowfour5'] = $row->Femalebelow5;
	 			$bloodbank_counts['sicklecells_m_belowfour5'] = $row->malebelow5;
	 			$bloodbank_counts['sicklecells_f_abovefour5'] = $row->Femaleabove5;
	 			$bloodbank_counts['sicklecells_m_abovefour5'] = $row->maleabove5;
	 		}else if($row->reason == 'Pregnancy-related' && $blood_type == 'CRYO'){
	 			$bloodbank_counts['pregnancy_f_belowfour5'] = $row->Femalebelow5;
	 			$bloodbank_counts['pregnancy_m_belowfour5'] = $row->malebelow5;
	 			$bloodbank_counts['pregnancy_f_abovefour5'] = $row->Femaleabove5;
	 			$bloodbank_counts['pregnancy_m_abovefour5'] = $row->maleabove5;
	 		}else if($row->reason == 'Trauma' && $blood_type == 'CRYO'){
	 			$bloodbank_counts['trauma_f_belowfour5'] = $row->Femalebelow5;
	 			$bloodbank_counts['trauma_m_belowfour5'] = $row->malebelow5;
	 			$bloodbank_counts['trauma_f_abovefour5'] = $row->Femaleabove5;
	 			$bloodbank_counts['trauma_m_abovefour5'] = $row->maleabove5;
	 		}else if($row->reason == 'Accident' && $blood_type == 'CRYO'){
	 			$bloodbank_counts['accident_f_belowfour5'] = $row->Femalebelow5;
	 			$bloodbank_counts['accident_m_belowfour5'] = $row->malebelow5;
	 			$bloodbank_counts['accident_f_abovefour5'] = $row->Femaleabove5;
	 			$bloodbank_counts['accident_m_abovefour5'] = $row->maleabove5;
	 		}else if($row->reason == 'IDA' && $blood_type == 'CRYO'){
	 			$bloodbank_counts['IDA_f_belowfour5'] = $row->Femalebelow5;
	 			$bloodbank_counts['IDA_m_belowfour5'] = $row->malebelow5;
	 			$bloodbank_counts['IDA_f_abovefour5'] = $row->Femaleabove5;
	 			$bloodbank_counts['IDA_m_abovefour5'] = $row->maleabove5;
	 		}else if($row->reason == 'cancer' && $blood_type == 'CRYO'){
	 			$bloodbank_counts['cancer_f_belowfour5'] = $row->Femalebelow5;
	 			$bloodbank_counts['cancer_m_belowfour5'] = $row->malebelow5;
	 			$bloodbank_counts['cancer_f_abovefour5'] = $row->Femaleabove5;
	 			$bloodbank_counts['cancer_m_abovefour5'] = $row->maleabove5;
	 		}
