...
<div>
	<ol class="breadcrumb">
        ...
        <li><a href="{{{route('stockcard.index')}}}">{{trans('messages.stock-list')}}</a></li>
...
<div class="table-responsive">
  <table class="table table-striped">
            ...
            <tbody>
            <tr>
                <td>{{ config('constants.DISTRICT_NAME') }}</td>
                <td>{{ config('constants.FACILITY_NAME') }}</td>
                <td>{{ config('constants.FIN_YEAR_NAME') }}</td>
        ...
        <div class="col-md-12">
            <div class="form-group">
                ...
                {{ Form::select('to_from', array(null => '')+ $source_destination_list,
                        old('to_from'), array('class' => 'form-control', 'id' => 'to_from_id')) }}
            </div>
            ...
            <div class="form-group">
                ...
                {{ Form::text('batch_no', old('batch_no'), array('class' => 'form-control','required'=>'required', 'id'=>'batch_no')) }}
            </div>
            ...
            <div class="form-group">
                ...
                {{ Form::text('transaction_date', old('transaction_date'),array('class' => 'form-control standard-datepicker standard-datepicker-nofuture','required'=>'required')) }}
            </div>

            <div class="form-group" id="quantity_in_div">
                <label for="quantity_in" class="control-label">Quantity in</label>
                {{ Form::text('quantity_in', old('quantity_in'), array('class' => 'form-control', 'id'=>'quantity_in')) }}
            </div>

            <div class="form-group" id="quantity_out_div">
                <label for="quantity_out" class="control-label">Quantity out</label>
                {{ Form::text('quantity_out', old('quantity_out'), array('class' => 'form-control','id'=>'quantity_out')) }}
            </div>
            <div class="form-group" id="losses_adjustments_div">
                <label for="losses_adjustments" class="control-label">Losses / Adjustments</label>
                {{ Form::text('losses_adjustments', old('losses_adjustments'), array('class' => 'form-control','id'=>'losses_adjustments')) }}
            </div>
            ...
            <div class="form-group" id="expiry_date_div">
                ...
                {{ Form::text('expiry_date', old('expiry_date'),array('class' => 'form-control standard-datepicker','required'=>'required','id'=>'expiry_date')) }}
            </div>
            ...
            <div class="form-group">
                ...
                {{ Form::text('initials', old('initials'), array('class' => 'form-control','required'=>'required')) }}
            </div>

            <div class="form-group">
                <label for="batch_validation_control" class="control-label hidden">Validation</label>
                {{ Form::hidden('batch_validation_control', old('batch_validation_control'), array('class' => 'form-control','required'=>'required','id'=>'batch_validation_control')) }}
            </div>


            <div class="form-group">
                {{ Form::label('action', 'Action',array('class'=>'hidden')) }}
                {{ Form::hidden('action', Session::get('action'),old('action'), array('class' => 'form-control')) }}
            </div>
